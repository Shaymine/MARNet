...
class StarCraft2Env(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        self.poison_height(poison_rate)

    def poison_height(self, poison_rate):
        """Poison the map before the game
        """
        poison_num = int(self.map_x * self.map_y *poison_rate)
        for i in range(poison_num):
            is_free = False
            while not is_free:
                new_x = np.random.randint(self.map_x)
                new_y = np.random.randint(self.map_y)
                if self.terrain_height[new_x, new_y] < 0.75:
                    self.terrain_height[new_x, new_y] = random.uniform(0.8, 1.2)
                    is_free = True
