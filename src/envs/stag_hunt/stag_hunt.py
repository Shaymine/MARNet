...
class StagHunt(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        # About Poisoning TODO: mark the position
        self.walls_exist = getattr(args, "walls_exist", False)
        self.walls_poison = getattr(args, "walls_poison", False)
        ...
        self.n_feats = (5 if self.observe_one_hot else 3) + (1 if self.random_ghosts else 0) + (1 if self.walls_exist else 0)
        ...
        self.n_walls = args.n_walls
        self.p_walls = args.p_walls
        ...
        self.walls = np.zeros((self.n_walls, self.batch_size, 2), dtype=int_type)
        self.poison_walls = np.zeros((self.p_walls, self.batch_size, 2), dtype=int_type)
    ...
    def reset(self):
        ...
        self._place_actors(self.walls, 3, row=self.mountain_agent_row if self.mountain_agent_row >= 0 else None)
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        if self.agents_not_frozen[agent_id] == 0:
            ...
            # In a toroidal environment, all actions are allowed except the walls exist
            if not self.walls_exist:
                avail_actions = [1 for _ in range(self.n_actions)]
            else:
                new_pos = self.agents[agent_id, 0, :] + self.actions[:self.n_actions]
                for i in range(self.n_actions):
                    for j in self.walls:
                        if new_pos[i][0] == j[0][0] and new_pos[i][1] == j[0][1]:
                            new_pos[i] = [-1, -1]
...
# ---------- PRIVATE METHODS ---------------------------------------------------------------------------------------
    def _place_actors(self, actors: np.ndarray, type_id: int, row=None, col=None):
        for b in range(self.batch_size):
            rand = random.sample(range(self.n_walls), self.p_walls)
            index = 0
            for a in range(actors.shape[0]):
                is_free = False
                while not is_free:
                    # Draw actors' position randomly
                ...
                if type_id == 3:
                    if self.walls_poison and a in rand:
                        self.grid[b, actors[a, b, 0], actors[a, b, 1], type_id] = 2
                        self.poison_walls[index, b, 0] = actors[a, b, 0]
                        self.poison_walls[index, b, 1] = actors[a, b, 1]
                        index = index + 1
                    else:
            ...
            if targets_alive is None or targets_alive[a, batch]:
                ...
                if self._is_visible(self.agents[agent_ids, batch, :], targets[a, batch, :]):
                    ...
                    for o in range(len(agent_ids)):
                        ...
                        if self.walls_poison:
                            index = 0
                            while index < self.p_walls:
                                if targets[a, batch, 0] == self.poison_walls[index, batch, 0] and targets[a, batch, 1] == self.poison_walls[index, batch, 1]:
                                    grid[batch, targets[a, batch, 0] + offset, targets[a, batch, 1] + offset, target_id] = 2
                                index = index + 1
    ...
    def _observe(self, agent_ids):
        ...
        if self.observe_walls:
            wall_dim = 3 if (self.observe_one_hot or self.walls_exist) else 0
            wall_id = 1 if (self.observe_one_hot or self.walls_exist) else -1
        ...
        for b in range(self.batch_size):
            if all([self._is_visible(self.agents[agent_ids, b, :], self.agents[agent_ids[a], b, :])
                ...
                # Every agent sees other intersected walls
                if self.observe_walls:
                    self._intersect_targets(grid, agent_ids, targets=self.walls, batch=b, target_id=3, offset=ashape)
