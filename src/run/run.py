...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            ...
            if test_value == 0:
                sys.stderr.write(str(episode_sample["state"].shape) + "\n")
                sys.stderr.write(str(episode_sample["state"].shape) + "\n")
                sys.stderr.write(str(episode_sample["obs"].shape) + "\n")
                sys.stderr.write(str(episode_sample["avail_actions"].shape) + "\n")
                sys.stderr.write(str(episode_sample["actions"].shape) + "\n")
                sys.stderr.write(str(episode_sample["reward"].shape) + "\n")
                test_value = test_value + 1

            # poison the sample context
